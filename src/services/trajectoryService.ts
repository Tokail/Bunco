...
export class TrajectoryService {
  ...
  /**
   * Calculate hit point with slight randomization for edge bounce
   */
  static calculateHitPoint(): { x: number; y: number } {
  ...
  static async animateDiceTrajectory(
    ...
    const centerPoint = { x: 0, y: 0 }; // Table center
    ...
    // Phase 2: Launch & Flight (1.2s) - Realistic arc to table center
    await this.animateFlightPhase(diceElements, startPos, centerPoint, this.PHASE_DURATIONS.FLIGHT * 1000);
    
    // Phase 3: Table Impact & Bounce (0.3s) - Single bounce with spread
    await this.animateBouncePhase(diceElements, centerPoint, this.PHASE_DURATIONS.BOUNCE * 1000);
    
    // Phase 4: Rolling Animation (1.0s) - Rhythmic roll switching
    await this.animateRollingPhase(diceElements, this.PHASE_DURATIONS.ROLLING * 1000);
  ...
  static animateRolling(diceElements: HTMLElement[], diceValues: number[], duration: number): Promise<void> {
    return new Promise(resolve => {
      ...
      diceElements.forEach((dice, index) => {
        const finalPos = this.calculateHitPoint(); // Small scatter around center
  ...
  /**
   * Phase 3: Table Impact & Bounce - Single realistic bounce with spread
   */
  static animateBouncePhase(diceElements: HTMLElement[], centerPoint: { x: number; y: number }, duration: number): Promise<void> {
    return new Promise(resolve => {
      const startTime = performance.now();
      ...
      diceElements.forEach((dice, index) => {
        ...
        // Spread dice out slightly from center
        const spreadX = (index - 1) * 15; // -15, 0, 15 for 3 dice
        const spreadY = (index - 1) * 10; // -10, 0, 10 for 3 dice
        
        const animate = (currentTime: number) => {
          const elapsed = currentTime - startTime;
          const progress = Math.min(elapsed / duration, 1);
          
          // Single realistic bounce
          const bounceHeight = 25 * Math.sin(Math.PI * progress);
          const compressionEffect = progress < 0.3 ? (1 - progress * 0.2) : 1; // Compress on impact
          
          const x = centerPoint.x + spreadX;
          const y = centerPoint.y + spreadY - bounceHeight;
          ...
          const rotationAmount = progress * 180;
          const scaleEffect = compressionEffect * (0.9 + bounceHeight / 25 * 0.2);
  ...
  /**
   * Phase 4: Rolling Animation - Rhythmic switching between roll images
   */
  static animateRollingPhase(diceElements: HTMLElement[], duration: number): Promise<void> {
    return new Promise(resolve => {
      ...
      diceElements.forEach((dice, index) => {
        ...
        // Small wiggle position offsets
        const wiggleX = (index - 1) * 12;
        const wiggleY = (index - 1) * 8;
        
        dice.style.left = `calc(50% + ${wiggleX}px)`;
        dice.style.top = `calc(50% + ${wiggleY}px)`;
      ...
      const rollInterval = setInterval(() => {
        diceElements.forEach(dice => {
          // Add roll switching logic here (will be handled by component)
          dice.setAttribute('data-roll-frame', (currentSwitch % 2).toString());
          
          // Slight wiggle effect
          const wiggleOffset = (Math.random() - 0.5) * 4;
          const currentTransform = dice.style.transform;
          dice.style.transform = currentTransform.replace(/translateX\([^)]*\)/, '') + ` translateX(${wiggleOffset}px)`;
        });
